...
2026-09-05 用户确认取消 ZCode 只能做后端的角色限制。本规格仍定义后端交付要求；当前任务明确包含前端时，ZCode 可以一并实现，按该任务的需求、设计和契约验收。仅执行本后端规格时，不自动增加前端功能。
...
- 当前任务明确包含的 `apps/web/**`、`apps/desktop/**` 页面、样式、组件、前端状态管理、产品文案及相关测试；这些工作不因执行者为 ZCode 而受限。
...
看到其他工作流已创建的文件时必须保留并适配；同一文件只有一个写入 owner，不得重置或覆盖他人的修改。
...
- 当前任务未要求的前端页面、Tauri UI、Stitch 设计或临时产品 UI。
...
- 本后端规格要求可测试的启动/停止和 WakePolicy 后端；当前任务明确包含桌面 UI 时，ZCode 也可实现自动登录启动的视觉开关和 Tauri 插件接线。
...
本后端规格要求登录跳转所需的后端能力和契约；当前任务包含前端时，ZCode 也可实现 Agent Console 侧的登录跳转 UI，沿用现有 dev-toolbox 登录页面。
...
- 前端修改均对应当前任务的明确需求，并完成相关验收；仅执行本后端规格时不额外增加前端工作。
- 未部署、未推送、未创建 tag。
...
11. 完成任务必须实质性扩大到未授权的 UI 范围或大范围重构 dev-toolbox。
...
前端工作以当前任务明确要求为准，ZCode 可以实现；仅执行本后端规格时不自动增加 UI。不要启动 Codex App Server，不要增加 MinIO/kkFileView/Redis/消息队列，不要执行 Git 写操作或生产部署。所有 Codex 真实验证使用专用测试任务且不得修改项目文件。后端交付必须到达前端可直接对接状态；当前任务包含前端时还须完成对应验收，而不是计划或脚手架状态。
